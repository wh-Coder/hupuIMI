...
class IconCom extends Component {
  ...
  onChangeText = (text) => {
    let arr = []
    iconArr.map((value) => {
      if(value.indexOf(text.toLocaleLowerCase()) > -1){
        arr.push(value)
      }
    })
        ...
        <FlatList
          ...
          renderItem={({item}) => (
            <View style={{flexDirection: 'row',borderBottomWidth: 1,borderBottomColor: '#ccc'}}>
              <Icon style={{padding: 20}} name={item} size={30} />
